...
-- Índice GIN (será ignorado se a coluna não existir - o índice é opcional)
...
-- ========== ADICIONAR FK SEPARADAMENTE (se a tabela pedidos_anymarket existir) ==========
ALTER TABLE pedidos_items 
DROP CONSTRAINT IF EXISTS pedidos_items_pedido_id_fkey;

ALTER TABLE pedidos_items 
ADD CONSTRAINT pedidos_items_pedido_id_fkey 
FOREIGN KEY (pedido_id) REFERENCES pedidos_anymarket(id) ON DELETE CASCADE;

-- ========== DADOS INICIAIS (ignora se já existirem) ==========
